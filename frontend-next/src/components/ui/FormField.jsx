...
export const FormField = ({ label, error, required, children, hint }) => (
  <div>
    <label className="block text-[10px] font-bold tracking-widest text-[#666666] uppercase mb-2.5">
...
export const TextInput = ({ error, className = '', ...rest }) => (
  <input
    className={`w-full px-4 py-3.5 rounded-lg text-xs outline-none transition-colors border focus:border-[#000000] ${className}`}
...
export const SelectInput = ({ error, className = '', children, ...rest }) => (
  <select
    className={`w-full px-4 py-3.5 rounded-lg text-xs outline-none transition-colors border cursor-pointer appearance-none focus:border-[#000000] ${className}`}
    style={{ ...baseInputStyle, borderColor: error ? '#EF4444' : baseInputStyle.borderColor }}
    {...rest}
  >
    {children}
