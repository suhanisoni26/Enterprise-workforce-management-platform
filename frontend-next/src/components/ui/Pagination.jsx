  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t" style={{ borderColor: 'var(--border-color, rgba(148,163,184,0.12))' }}>
      <p className="text-[11px] text-[#64748B] font-medium">
        Showing <span className="text-[#CBD5E1] font-bold">{start}â€“{end}</span> of{' '}
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          disabled={page <= 1}
          className="w-8 h-8 flex items-center justify-center rounded-lg border border-[rgba(148,163,184,0.14)] text-[#666666] hover:bg-white/5 hover:text-black disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        >
          ...
          n === '...' ? (
            ...
            <button
              ...
              className={`w-8 h-8 flex items-center justify-center rounded-lg text-[11px] font-bold transition-colors ${
                n === page ? 'text-black' : 'text-[#666666] hover:bg-white/5 hover:text-black border border-[rgba(148,163,184,0.14)]'
              }`}
              style={n === page ? { backgroundColor: '#000000' } : {}}
            >
              {n}
            </button>
          )
