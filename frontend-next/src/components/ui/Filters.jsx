...
export const SearchInput = ({ value, onChange, placeholder = 'Search...', className = '' }) => (
  <div className={`relative ${className}`}>
    <HiOutlineSearch className="absolute left-3.5 top-1/2 -trangray-y-1/2 w-4 h-4 text-[#64748B]" />
    <input
      type="text"
      value={value}
    ...
    {value && (
      <button
        onClick={() => onChange('')}
        className="absolute right-3 top-1/2 -trangray-y-1/2 text-[#64748B] hover:text-black"
      >
...
export const SelectFilter = ({ value, onChange, options, placeholder = 'All' }) => (
  <div className="relative">
    ...
    <HiOutlineChevronDown className="absolute right-3 top-1/2 -trangray-y-1/2 w-3.5 h-3.5 text-[#64748B] pointer-events-none" />
  </div>
);
