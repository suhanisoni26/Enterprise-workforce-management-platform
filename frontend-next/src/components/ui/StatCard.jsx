 ...
 *   trend={4.2} trendLabel="vs last month" accent="#000000" />
 */
const StatCard = ({ label, value, icon: Icon, trend, trendLabel, accent = '#000000', suffix, loading = false }) => {
  if (loading) {
    return (
      <div
        className="p-6 rounded-2xl border min-h-[152px] flex flex-col justify-between"
        style={{ backgroundColor: 'var(--surface, #FFFFFF)', borderColor: 'var(--border-color, rgba(148,163,184,0.12))' }}
      >
  ...
  return (
    ...
    >
      ...
      <h2 className="text-4xl font-bold text-[#111111] my-3" style={{ fontFamily: 'var(--font-display)' }}>
        {value}
